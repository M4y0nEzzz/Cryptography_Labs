...
```txt
    ...
    x ^= (x << 5) & 0xFFFFFFFF
```
, что обеспечивает полный период 2³²-1.

- **Инициализация генератора** происходит по следующему прицнипу:
```txt
...
независимо с использованием одного и того же ключа.
- **Особенности**: независимое шифрование каждого блока, при этом если блоки открытого текста одинаковые, то и
...
- **XOR**: IV длиной 16 байт генерируется случайно для каждого шифрования. Сохраняется в метаданных (файл `*_meta.json`).
...
Мы вычисляем следующие метрики для каждого алгоритма и изображения:
...
H(X) = -Σ p(x) × log₂(p(x))
```
, где p(x) - вероятность появления значения х в данных.
...
1) ![Checkerboard_src](C:\Users\Михаил\PycharmProjects\Cryptography_Labs\Lab_1\results\checkerboard_xor_hist_src.png)
